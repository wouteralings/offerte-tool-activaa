...
// ---------------------------------------------------------------------------
// Mockdata — valt terug wanneer de echte Dynamics 365-data (via /api/klanten)
// nog niet beschikbaar is, bijv. bij lokale ontwikkeling.
// ---------------------------------------------------------------------------
...
export default function OffertetoolApp() {
  ...
  // Klanten ophalen uit Dynamics via de Azure Function; valt terug op voorbeelddata
  // als het endpoint (nog) niet bereikbaar is (bijv. lokale ontwikkeling).
  const [klanten, setKlanten] = useState(MOCK_KLANTEN);
  const [klantenLaden, setKlantenLaden] = useState(true);
  const [klantenFout, setKlantenFout] = useState(null);

  useEffect(() => {
    let actief = true;
    fetch("/api/klanten")
      .then((res) => {
        if (!res.ok) throw new Error("Kon klanten niet ophalen");
        return res.json();
      })
      .then((data) => {
        if (!actief) return;
        const omgezet = data.map((k, i) => ({
          id: `acc-${i}`,
          naam: k.naam || "(naam onbekend)",
          plaats: k.plaats || "",
          contact: "",
          email: k.email || "",
          segment: "",
      ...
      .catch((err) => {
        if (actief) setKlantenFout(err.message);
      })
      .finally(() => {
        if (actief) setKlantenLaden(false);
      });
    return () => {
      actief = false;
    };
    ...
    >
      ...
      <div style={{ maxWidth: 1040, margin: "0 auto", padding: "36px 24px 60px" }}>
        ...
        {stap === "klant" && (
          ...
          >
            {klantenLaden && (
              <div style={{ textAlign: "center", padding: 20, color: "#8A9089", fontSize: 13.5 }}>
                Klanten worden opgehaald uit Dynamics…
              </div>
            )}
            {klantenFout && (
              <div style={{ padding: "10px 14px", background: "#FBF2EC", borderRadius: 8, color: "#B14A2E", fontSize: 12.5, marginBottom: 14 }}>
                Kon geen klanten ophalen uit Dynamics ({klantenFout}). Voorbeelddata wordt getoond.
              </div>
            )}
                    ...
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 12.5, color: "#5B6259", marginTop: 2 }}>
                          {[k.contact, k.plaats, k.segment].filter(Boolean).join(" · ")}
                        </div>
                      </div>
                    </div>
                  </button>
